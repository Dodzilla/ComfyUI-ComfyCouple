...
def get_masks_from_q(masks, q, original_shape):
    # Detect downsample rate from attention spatial resolution
    area = q.shape[1]
    down_sample_rate = None
    
    for r in (1, 2, 4, 8, 16):
        if (original_shape[2] // r) * (original_shape[3] // r) == area:
            down_sample_rate = r
            break
    
    if down_sample_rate is None:
        raise ValueError(f"Unsupported attention area {area} for original shape {original_shape[2]}x{original_shape[3]}")
    ...
    for mask in masks:
        if isinstance(mask,torch.Tensor):
            ...
            # Explicitly match dtype/device to q for absolute safety across backends
            mask_downsample = mask_downsample.to(dtype=q.dtype, device=q.device)
            # Optional clamp to keep mask values well-behaved before normalization
            mask_downsample.clamp_(0, 1)
            ...
            ret_masks.append(torch.ones_like(q, dtype=q.dtype, device=q.device))
...
class AttentionCouple:
    ...
    def attention_couple(self, model, positive, negative, mode, debug=False):
        self.debug = debug
        if self.debug:
        ...
        for cond_idx, conditions in enumerate([new_negative, new_positive]):
            ...
            if len(conditions) != 1:
                ...
                try:
                    ...
                    mask_list = [
                        cond[1]["mask"].to(device, dtype=dtype) * float(cond[1].get("mask_strength", 1.0))
                    ...
                    # Normalize across conditions; guard against div-by-zero
                    sum_ = mask_norm.sum(dim=0)
                    mask_norm = mask_norm / sum_.clamp_min(1e-6)
                    if self.debug:
                    ...
                    # Remove mask metadata from ALL entries to avoid double application downstream
                    for c in conditions:
                        if len(c) > 1:
                            c[1].pop("mask", None)
                            c[1].pop("mask_strength", None)
                            c[1].pop("set_area_to_bounds", None)  # Also remove this helper flag
                    if self.debug:
        ...
        # Use the post-processed conditioning lengths to ensure they match what we'll actually consume
        self.conditioning_length = (
            len(self.negative_positive_conds[0]),
            len(self.negative_positive_conds[1]),
        )
        if self.debug:
        ...
        # Robust block discovery that works across different model variants
        def patch_block_list(blocks, key_prefix):
            for bid, block in enumerate(blocks):
                # Many UNet blocks are tuples/lists; attn module is often at [1]
                if len(block) > 1 and hasattr(block[1], "transformer_blocks"):
                    depth = len(block[1].transformer_blocks)
                    for t in range(depth):
                        set_model_patch_replace(
                            new_model,
                            self.make_patch(block[1].transformer_blocks[t].attn2),
                            (key_prefix, bid, t)
                        )
        
        # Patch input blocks
        patch_block_list(new_model.model.diffusion_model.input_blocks, "input")
        
        # Patch middle block
        mb = new_model.model.diffusion_model.middle_block
        if len(mb) > 1 and hasattr(mb[1], "transformer_blocks"):
            for t in range(len(mb[1].transformer_blocks)):
                set_model_patch_replace(
                    new_model, 
                    self.make_patch(mb[1].transformer_blocks[t].attn2), 
                    ("middle", 0, t)
                )
        
        # Patch output blocks  
        patch_block_list(new_model.model.diffusion_model.output_blocks, "output")
        
        if self.debug:
            print(f"[AttentionCouple] Model patching complete")
        # Return full conditioning lists to maintain downstream compatibility
        return (new_model, new_positive, new_negative)
    ...
    def make_patch(self, module):
        def patch(q, k, v, extra_options):
            ...
            # Note: We stack multiple condition contexts along BATCH (dim=0) with ordering:
            # [cond0_batch0..b-1, cond1_batch0..b-1, ...] and repeat q with the same length
            # multiplier, so shapes align at (length*b, ...) for attention computation.
            ...
            for i, c in enumerate(cond_or_uncond):
                ...
                # k, v are already shaped (length*b, context_tokens, dim) in the same grouping order as q_target.
                # No reconstruction necessary - safe_concat_conds already stacked them along batch dim correctly.
